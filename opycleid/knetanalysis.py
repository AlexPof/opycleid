...
class KNet:
    ...
    def __init__(self,category):
        ...
        Variables
        ----------
        - vertices : dictionary, values are elements in the K_Net category
        - generating_edges : dictionary, values are 3-tuples (start,end,op)
        - category : an instance of a CategoryAction class
        """

        if not isinstance(category,CategoryAction):
            raise Exception("Not a valid category action\n")
        else:
            self.vertices = {}
            self.generating_edges = {}
            self.category = category
            self.knet_categoryactionfunctor = None
    ...
    def set_vertices(self,list_vertices):
        ...
        Parameters
        ----------
        list_vertices : a dictionary XXXX SHOULD
        ...
        self.vertices = {}
        self.generating_edges = {}
        for vertex_name,vertex_element in sorted(list_vertices.items()):
            for name_obj,obj in self.category.get_objects():
                if not obj.is_in(vertex_element):
                    raise Exception("Element "+str(vertex_element)+" is not in the category action\n")
            self.vertices[k] = CatObject(vertex_name,[vertex_element])

        self.knet_categoryactionfunctor = None

    def add_generating_edges(self,list_edges):
        """Add generating edges to a K_Net.
        Checks if:
        ...
        list_edges : list of 3-tuples (start,end,op), where
                - start is the key of the starting vertex
                - end is the key of the ending vertex
                - op is an operation in the K_Net category

        Returns
        -------
        None
        """
        for vertex_source,vertex_target,operation in list_edges:

            for edge_start,edge_end,edge_op in self.edges.values():
                if edge_start == vertex_source and edge_end == vertex_target:
            ...
            element_source = self.vertices[vertex_source]
            element_target = self.vertices[vertex_target]

            if not operation in self.category.get_operation(element_source,element_target):
                raise Exception(str(operation)+" is not a valid operation for the corresponding vertices\n")

            morphism = CatMorphism("h_{}".format(len(self.edges)),
                                    self.vertices[vertex_source],
                                    self.vertices[vertex_target]
                                    )

            self.edges[len(self.edges)] = CatMorphism(),

            (vertex_source,vertex_target,operation)

        self.knet_categoryactionfunctor = None

    def build_knet(self):
        list_objects = []
        for vertex_name,vertex_element in self.vertices.items():
            list_objects.append(CatObject(vertex_name,[vertex_element]))
